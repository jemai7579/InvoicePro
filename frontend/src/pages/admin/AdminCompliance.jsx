...
const AdminCompliance = () => {
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (filter === 'missing_dossier') return row.missingMatriculeFiscal || row.missingLegalRepresentative;
    if (filter === 'missing_signature') return row.missingSignatureConfiguration;
    if (filter === 'missing_ttn') return row.missingTtnConfiguration;
    if (filter === 'rejected') return row.rejectedInvoices > 0;
    if (filter === 'ready') return row.productionReady;
    return true;
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <select value={filter} onChange={(event) => setFilter(event.target.value)} className="rounded-2xl border border-slate-100 bg-white px-4 py-3 text-sm outline-none">
            <option value="all">Toutes conformites</option>
            <option value="missing_dossier">Dossier manquant</option>
            <option value="missing_signature">Signature manquante</option>
            <option value="missing_ttn">TTN manquant</option>
            <option value="rejected">Factures rejetees</option>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <Card><div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Bloquees prod</div><div className="mt-2 text-2xl font-black text-rose-600">{rows.filter((row) => row.blockedByProductionRules).length}</div></Card>
      </div>

      <Card title="Fournisseurs de signature" subtitle="Statut global sans exposer certificats, PINs ou cles privees">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {['mock', 'external', 'HSM', 'certificate'].map((provider) => (
            <div key={provider} className="rounded-2xl bg-slate-50 border border-slate-100 px-4 py-4">
              <div className="text-sm font-black text-slate-900">{provider}</div>
              <div className="mt-2 text-xs text-slate-500">Entreprises: {rows.filter((row) => (row.signatureProvider || '').toLowerCase().includes(provider.toLowerCase()) || (provider === 'certificate' && row.signatureConfigured)).length}</div>
              <Badge className="mt-3" variant={provider === 'mock' ? 'warning' : 'secondary'}>{provider === 'mock' ? 'Demo non legal' : 'A configurer selon fournisseur'}</Badge>
            </div>
          ))}
        </div>
      </Card>
