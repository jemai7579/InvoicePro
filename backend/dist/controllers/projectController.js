...
exports.respondToProject = exports.sendProject = exports.updateProject = exports.createProject = exports.getReceivedProjects = exports.getProjects = void 0;
...
const isProjectRequest = (request) => getJsonObject(request?.data).kind === PROJECT_KIND;
const normalizeProjectStatus = (data) => {
...
const getPrivateProjectClient = async (companyId) => {
...
const findAcceptedContact = async (companyId, targetCompanyId) => {
    const requests = await prisma_1.default.invoiceRequest.findMany({
        where: {
            OR: [
...
const buildSharedContactPayload = async (companyId, payload) => {
    ...
    const targetCompany = await prisma_1.default.company.findUnique({
...
const getReceivedProjects = async (req, res) => {
    try {
        const companyId = String(req.company.id);
...
exports.getReceivedProjects = getReceivedProjects;
const createProject = async (req, res) => {
    try {
        ...
        const project = await prisma_1.default.$transaction(async (tx) => {
            const projectReference = await (0, numberingService_1.generateBusinessNumber)(tx, companyId, 'PROJECT');
...
const updateProject = async (req, res) => {
    try {
        const companyId = String(req.company.id);
        const existing = await prisma_1.default.invoiceRequest.findFirst({
        ...
        const sharing = await buildSharedContactPayload(companyId, {
            ...req.body,
            sharedWithCompanyId: req.body.sharedWithCompanyId === undefined
...
const sendProject = async (req, res) => {
    try {
        ...
        const project = await prisma_1.default.invoiceRequest.findFirst({
            where: { id: req.params.id, companyId },
            include: { client: true, company: true },
        });
    ...
    `;
        const info = await (0, mailer_1.sendEmail)(data.recipientEmail, subject, html);
        await (0, notificationHelper_1.createNotif)(data.connectedCompanyId, 'Nouveau projet recu', `${company.name} vous a envoye le projet "${data.title || 'Sans titre'}".`, 'PROJECT_RECEIVED');
        res.status(200).json({
...
const respondToProject = async (req, res) => {
    try {
        ...
        await (0, notificationHelper_1.createNotif)(project.companyId, accepted ? 'Projet accepte' : 'Projet refuse', `${req.company.name} a ${accepted ? 'accepte' : 'refuse'} le projet "${data.title || 'Sans titre'}".`, accepted ? 'PROJECT_ACCEPTED' : 'PROJECT_REFUSED');
        res.status(200).json(toProjectDto(updated));
    }
    catch (error) {
...
exports.respondToProject = respondToProject;
