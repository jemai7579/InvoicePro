...
type PdfType = 'FACTURE' | 'DEVIS';

type ComplianceMetadata = {
  complianceStatus?: string;
  complianceLabelFr?: string;
...
export const getPdfDocumentNumber = (docData: any, type: PdfType) =>
  type === 'DEVIS' ? getDevisVisibleNumber(docData) : getInvoiceVisibleNumber(docData);

export const getPdfFileSafeNumber = (docData: any, type: PdfType) =>
  sanitizeBusinessNumberForFileName(getPdfDocumentNumber(docData, type));

function groupByTva(lines: any[]) {
...
export const generatePdf = async (
  docData: any,
  type: PdfType = 'FACTURE',
  compliance?: ComplianceMetadata
): Promise<Buffer> => {
  const createdAt = new Date(docData.createdAt);
  const ref = getPdfDocumentNumber(docData, type);
  ...
  return new Promise((resolve, reject) => {
    ...
    drawParty(
      rightX,
      type === 'DEVIS' ? 'DESTINATAIRE' : 'CLIENT',
      client.name,
    ...
    ensureSpace(32);
    const words = numberToWordsTND(Number(docData.netToPay || 0));
